...
def check_password():
    ...
    def password_entered():
        ...
        if user_data:
            if check_hashed_password(password, user_data['hashed_password']):
    ...
    if not st.session_state.authenticated:
        ...
        if not get_user_from_db("admin_setup_check"):
            st.warning("⚠️ No hay usuarios registrados. Crea el primer usuario administrador.")
            with st.form("Initial_Admin_Setup"):
                st.text_input("Usuario Administrador:", key="admin_user_setup").strip()
                st.text_input("Contraseña:", type="password", key="admin_password_setup").strip()
                if st.form_submit_button("Crear Administrador Inicial", type="primary"):
                    if add_user_to_db(st.session_state["admin_user_setup"], st.session_state["admin_password_setup"], is_admin=True):
                        ...
                        st.rerun()
            st.stop() # Detiene la ejecución si estamos en setup
        ...
        if "username_input" in st.session_state and st.session_state["username_input"]:
            if not st.session_state.authenticated and st.session_state["username_input"] != "admin_setup_check":
                st.error("Usuario o contraseña incorrectos.")
        
        return False # No continuar con la aplicación
    
    return True # Continuar con la aplicación

# Llama a la función de autenticación. Si es False, detiene la ejecución.
...
# ====================================================================
#               CÓDIGO DE LA APLICACIÓN PRINCIPAL (DATOS)
# ====================================================================

# --- Funciones de Datos (Insertar, Obtener, Eliminar, Actualizar) ---
...
# Lógica de verificación de conexión (solo muestra el mensaje si está OK)
try:
    conn = get_db_connection()
...
if nombres_especies_existentes:
    # Si hay especies, el placeholder es la primera opción
    opciones_especie_select = ["-- Seleccionar Especie --"] + opciones_especie_select
    default_index_especie = 0
else:
    # Si no hay especies, seleccionamos "Nueva Especie" por defecto.
    default_index_especie = 0
...
if nombres_fases_existentes:
    ...
    # Si no hay fases, seleccionamos "Nueva Fase" por defecto.
    default_index_fase = 1


# TABS: Se añade el Tab 4 (Admin)
...
# ----------------- TAB 1: REGISTRAR INGREDIENTE (CON FIX) -----------------
with tab1:
    ...
    with st.form(key="form_registrar_medio"):
        ...
        especie = None # Inicializamos la variable que se usará para la inserción
        ...
        if is_new_especie:
            ...
            if especie_input:
                ...
                # Solo mostramos el error si el usuario intenta guardar con el campo vacío
                if 'submit_button' in st.session_state and st.session_state.submit_button:
                     st.error("Por favor, introduce un nombre para la Nueva Especie.")
                especie = None
        else:
        ...
        if is_new_fase:
            ...
            if fase_input:
                ...
                # Solo mostramos el error si el usuario intenta guardar con el campo vacío
                if 'submit_button' in st.session_state and st.session_state.submit_button:
                    st.error("Por favor, introduce un nombre para la Nueva Fase de Cultivo.")
                fase_cultivo = None
        else:
        ...
        if not especie and especie_seleccionada == "-- Seleccionar Especie --":
             st.warning("⚠️ Debes seleccionar o ingresar la Especie de Planta.")
        if not fase_cultivo and fase_seleccionada == "-- Seleccionar Fase --":
             st.warning("⚠️ Debes seleccionar o ingresar la Fase de Cultivo.")
            ...
            with st.form(key="form_editar_medio", clear_on_submit=False):
                ...
                with col_update:
                    if st.form_submit_button("💾 Guardar Cambios", type="primary"):
                        actualizar_medio_cultivo(st.session_state.edit_id, especie_edit, fase_cultivo_edit, ingrediente_edit, concentracion_edit, unidad_edit)
                        st.session_state.edit_id = None 
                        st.rerun()
        ...
        for index, row in df_filtrado.iterrows(): 
            ...
            with col3:
                with st.form(key=f"delete_form_{row['id']}", clear_on_submit=False):
                    st.form_submit_button("🗑️", type="primary", on_click=eliminar_medio_cultivo, args=(row['id'],))
